...
class SQLiteStore:
    ...
    def get_bookmarks(self, filters: dict[str, Any] | None = None) -> pl.DataFrame:
    ...
    def update_bookmark(self, bookmark_id: int, update_data: dict[str, Any]):
    ...
    def delete_bookmark(self, bookmark_id: int):
        self.cursor.execute(
            f"DELETE FROM {self.table_name} WHERE id = ?", (bookmark_id,)
        )
        self.conn.commit()
